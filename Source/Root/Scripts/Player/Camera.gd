...
func _process(delta):
	...
	var mouse_pos = get_viewport().get_mouse_position() / window_scale - (game_size/2) + player.global_position
	
	# Using a lerp, the cameras position is moved towards the mouse position
	var cam_pos = lerp( player.global_position, mouse_pos, 2)
	...
	#get_viewport().material.set_shader_param("cam_offset", cam_subpixel_pos )
